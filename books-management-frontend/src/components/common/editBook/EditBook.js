...
function EditBook() { 
    ...
    useEffect(()=>{
        const fetchingAuthorsAndGenres= async ()=>{
         ...
         try{
          const authorUrl= "http://localhost:3001/addbook/authors";
          const genresurl ="http://localhost:3001/addbook/genres"; 
          const previousUrl= `http://localhost:3001/book-details/${bookId}`;
     ...
     const onSubmitUpdateBooks= async (e)=>{
      ...
      try{
        const addurl=`http://localhost:3001/edit-book/${bookId}`
        const options={ 
          method:"PUT",
          headers:{
