...
function SearchResults() {
  ...
  useEffect(() => {
    const fetchSearchResults = async () => {   
      try {
        const queryUrl = `http://localhost:3001/search?searchText=${searchtext}&authorId=${selectAuthor}&genreId=${selectGenre}`;
    ...
    fetchSearchResults();
  }, [searchtext, selectAuthor, selectGenre]);
  ...
  const handleDelete = async (bookId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/delete-book/${bookId}`, {
        method: 'DELETE',
      });
      if (response.ok) {
